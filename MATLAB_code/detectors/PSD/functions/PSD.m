function detectedLabels = PSD(id)
% PSD loads the selected audio file, performs PSD-based segmentation,
% applies noise tracking, adaptive thresholding, and post-processing to
% detect ultrasonic vocalizations. It returns the detected labels.
%
% Input:
%   id - a number from 1 to 6 that selects one of the available audio files.
%
% Output:
%   detectedLabels - a struct array with fields 'StartTime' and 'EndTime'
%                    corresponding to the detected vocalization events.

    %% Select Audio File Based on id
    switch id
        case 1
        ...
        otherwise
            error('Invalid id. Must be an integer between 1 and 6.');
    end
    disp("------------------------");
    disp(audioFile);
    ...
    minDuration = 0.0005;      % Minimum duration of an event to keep (s) (0.5 ms)
    maxGap      = 0.001;       % Maximum gap between events to merge (s) (1 ms)
